...
static void sec2msf(unsigned int s, unsigned char *msf) {
	msf[0] = s / 75 / 60;
	s = s - msf[0] * 75 * 60;
	msf[1] = s / 75;
	s = s - msf[1] * 75;
...
static void startCDDA(unsigned int offset) {
	...
	threadid = CreateThread(NULL, 0, cddathread, NULL, CREATE_SUSPENDED, NULL);
	...
	while (fgets(linebuf, sizeof(linebuf), fi) != NULL) {
		...
		if (!strcmp(token, "TRACK")) {
			...
			if (ti[numtracks].type == CDDA) {
				...
				t += 2 * 75;
				sec2msf(t, (unsigned char *)&ti[numtracks].start);
...
static void iso_read_sector(unsigned char *time, unsigned char * cdbuffer) {
	if (subChanMixed) {
...
unsigned int cache_start_addr = 0;

sector_cache_t sector_cache[cache_size];

static sector_cache_t * sect_in_cache(unsigned char * time) {
	int i;	
	sector_cache_t * entry;
	// Parse the cache
	for(i = 0; i < cache_size; i ++) {
		entry = &sector_cache[i];
		// if found in cache use it
		if ( (entry->time[0] == time[0]) && 
				(entry->time[1] == time[1]) && 
				(entry->time[2] == time[2]) 
			)
		{
...
static DWORD dataThread(void*) {
	sector_cache_t * entry;
	unsigned char curTime[3];
	int i;
	unsigned int old_cache_start_addr = 0;

	while(1) {
		// if we are here it mean that the data is not in cache
		WaitForSingleObject(hDataLock, INFINITE);

		curTime[0] = neededTime[0];
		curTime[1] = neededTime[1];
		curTime[2] = neededTime[2];

		// for speed up read the 1st entry and release lock
		entry = &sector_cache[0];
		iso_read_sector(curTime, entry->buf);
		entry->time[0] = curTime[0];
		entry->time[1] = curTime[1];
		entry->time[2] = curTime[2];
		...
		for(i = 1; i < cache_size; i ++) {
			// look if it's not already cached
			if (sect_in_cache(curTime) == 0) {
				entry = &sector_cache[i];
...
static void initDataThread() {
	...
	HANDLE threadid = CreateThread(NULL, 0, dataThread, NULL, CREATE_SUSPENDED, NULL);
	
	hDataLock = CreateMutex(NULL, FALSE, "read thread lock");

	XSetThreadProcessor(threadid, 5);
	//SetThreadPriority(threadid ,THREAD_PRIORITY_HIGHEST);
	ResumeThread(threadid);
}

static unsigned char *  getFromCache(unsigned char * time) {
	sector_cache_t * entry;
	int i;
	int found = 0;
	...
	if (entry  == 0) {
		// wait a few ...
		Sleep(1);
		// refetch from cache until we got it ...
		printf("getFromCache: %02x:%02x:%02x\n", neededTime[0], neededTime[1], neededTime[2]);
		return getFromCache(time);
	}
...
static long CALLBACK ISOreadTrack(unsigned char *time) {
	...
	if(0) {
		iso_read_sector(time, cdbuffer);
	} else {
		unsigned char * cache = getFromCache(time);
		memcpy(cdbuffer ,cache, CD_FRAMESIZE_RAW);
	}
