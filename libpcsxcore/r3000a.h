...
static __inline u32 * Read_ICache(u32 pc, boolean isolate) {
	u32 pc_bank, pc_offset, pc_cache;
	// u32 * __restrict iaddr_cache;
	// u32 * __restrict icode_cache;
	// u32 * __restrict r;
	u8 * __restrict IAddr, * __restrict ICode;
	...
	if (!psxRegs.ICache_valid) {
		XMemSet(psxRegs.ICache_Addr, 0xff, sizeof(psxRegs.ICache_Addr));
		XMemSet(psxRegs.ICache_Code, 0xff, sizeof(psxRegs.ICache_Code));
		...
		if (SWAP32(*(u32 *)(IAddr + pc_cache)) == pc_offset) {
			...
			if (!isolate) {
				...
				*(u32 *)(IAddr + pc_cache + 0xc) = SWAP32(pc_offset + 0xc);
				...
				/*
				iaddr_cache = (u32 *)(IAddr + pc_cache);
				icode_cache = (u32 *)(ICode + pc_cache);
				
				*(u32 *)(iaddr_cache+0x0) = SWAP32(pc_offset + 0x0);
				*(u32 *)(iaddr_cache+0x4) = SWAP32(pc_offset + 0x4);
				*(u32 *)(iaddr_cache+0x8) = SWAP32(pc_offset + 0x8);
				*(u32 *)(iaddr_cache+0xC) = SWAP32(pc_offset + 0xc);
				*/

				// opcode line
				pc_offset = pc & ~0xf;
				
				//__dcbt(pc_cache,ICode);
				...
				*(u32 *)(ICode + pc_cache + 0xc) = psxMu32ref(pc_offset + 0xc);
				...
				/*
				*(u32 *)(icode_cache+0x0) = psxMu32ref(pc_offset + 0x0);
				*(u32 *)(icode_cache+0x4) = psxMu32ref(pc_offset + 0x4);
				*(u32 *)(icode_cache+0x8) = psxMu32ref(pc_offset + 0x8);
				*(u32 *)(icode_cache+0xC) = psxMu32ref(pc_offset + 0xC);
				*/
			}
			// normal code
			return (u32 *)PSXM(pc);
