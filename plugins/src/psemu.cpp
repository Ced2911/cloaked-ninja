...
extern "C" unsigned short CALLBACK SPUgetOne(unsigned long val)
{
...
extern "C" void CALLBACK SPUputOne(unsigned long val,unsigned short data)
{
 if(spuAddr!=0xffffffff)
  {
   SPUwriteDMA(data);
...
extern "C" void CALLBACK SPUplaySample(unsigned char ch)
{
}

extern "C" void CALLBACK SPUsetAddr(unsigned char ch, unsigned short waddr)
{
 s_chan[ch].pStart=spuMemC+((unsigned long) waddr<<3);
}

extern "C" void CALLBACK SPUsetPitch(unsigned char ch, unsigned short pitch)
{
 SetPitch(ch,pitch);
}

extern "C" void CALLBACK SPUsetVolumeL(unsigned char ch, short vol)
{
 SetVolumeR(ch,vol);
}
