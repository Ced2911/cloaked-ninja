...
#else
	sprintf(szTxt,"game:\\pcsx%04d.txt",iNum);
#endif
...
#else
		sprintf(filename, "%s/pcsx%04ld.bmp", "game:\\", snapshotnr);
#endif
...
long CALLBACK GPUinit()                                // GPU INIT
{
	...
	psxVSecure = (unsigned char *)XPhysicalAlloc(
		(iGPUHeight*2)*1024 + (1024*1024),MAXULONG_PTR, 
...
void ThUpdateDisplay(void *param){
	ParamUpdateDisplay * pParam = (ParamUpdateDisplay*)param;
	ThBufferSwap(pParam);
	Core0Free(pParam->psxuvw);
	Core0Free(pParam);
}

#define PSXUVW_SIZE (2097152) //2Mo
#define PSXUVW_SIZE ((iGPUHeight*2)*1024 + (1024*1024))

void updateDisplay(void){
	ParamUpdateDisplay * pParam;

	if(psxVuw==NULL)
		return;

	pParam = malloc(sizeof(ParamUpdateDisplay));
	
	pParam->PreviousPSXDisplay = PreviousPSXDisplay;
	pParam->PSXDisplay = PSXDisplay;
	pParam->psxuvw = (unsigned short *)malloc(PSXUVW_SIZE);

	if(pParam->psxuvw==NULL)
	{
	...
	//XMemCpy(pParam->psxuvw, psxVuw, PSXUVW_SIZE);
	memcpy(pParam->psxuvw, psxVuw, 1572864);//1.5mo
	
	//Sleep(10);

	AddFunc(ThUpdateDisplay,pParam);
}
#endif
////////////////////////////////////////////////////////////////////////
...
#if 0
typedef struct sDmaParam{
	uint32_t * pMem;
	int iSize;
}DmaParam;

/** function qui tourne dans un thread **/
void ThGPUwriteDataMem(void * pParam){
	DmaParam * pDmaParam = (DmaParam*) pParam;
	__GPUwriteDataMem(pDmaParam->pMem, pDmaParam->iSize);
	Core0Free(pDmaParam->pMem);
	Core0Free(pParam);
}

/** Function qui va remplacer **/
void GPUwriteDataMem(uint32_t * pMem, int iSize){
	//Creation du param envoyer a la function threader et sauvegarde l'adress memoire du dma
	DmaParam * pParam = (DmaParam*)malloc(sizeof(DmaParam));
	...
	pParam->iSize = iSize;
	pParam->pMem = (uint32_t*)malloc(iSize*sizeof(uint32_t));
	...
	XMemCpy(pParam->pMem, pMem, iSize*sizeof(uint32_t));
	
	//Ajoute en queue
	AddFunc(ThGPUwriteDataMem,(void*)pParam);
}
#endif
////////////////////////////////////////////////////////////////////////
