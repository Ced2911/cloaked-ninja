...
void UploadTexWndPal(int mode,short cx,short cy)
{
 ...
 glColorTable(GL_TEXTURE_2D,GL_RGBA8,iSize,
   ...
   if(ts->used)
    {
     if(ts->pos.l==npos.l &&
        ...
        ts->textureMode==TextureMode)
      {
       ...
       else if(glColorTable && TextureMode!=2)
        {
...
GLuint SelectSubTextureS(int TextureMode, uint32_t GivenClutId) 
{
 ...
 if(firstTime) {
	...
	reptex = loadtga("d:/test.tga", true);
	firstTime =0;
 } 
