...
#define SOFT_CALL if(0)

////////////////////////////////////////////////////////////////////////                                          
// defines
////////////////////////////////////////////////////////////////////////

#define DEFOPAQUEON  BK_OpaqueOn();                              
#define DEFOPAQUEOFF BK_OpaqueOff();
...
uint8_t        ubGloAlpha;                             // texture alpha
uint8_t        ubGloColAlpha;                          // color alpha
...
void primBlkFill(unsigned char * baseAddr)
{
	...
	if(ClipVertexListScreen())                           
	{
		...
		{
			// Fullscreen clear
			uint32_t fillColor = GETLE32(&gpuData[0]);
			BK_ScissorDisable();
			BK_Clear(uiBufferBits, RED(fillColor), GREEN(fillColor), BLUE(fillColor), 0xff);
			gl_z=0.0f;
	...
	if(iOffscreenDrawing)
	{
		...
		if(bDrawOffscreen4())
		{
			if(!(iTileCheat && sprtH==32 && gpuData[0]==0x60ffffff)) // special cheat for certain ZiNc games
			{
                ...
                SOFT_CALL FillSoftwareAreaTrans(lx0, ly0, lx2, ly2,
...
void primPolyF4(unsigned char *baseAddr)
{
	...
	if(iOffscreenDrawing)
	{
		...
		{
			InvalidateTextureAreaEx();   
			SOFT_CALL drawPoly4F(GETLE32(&gpuData[0]));
		}
	}
...
void primPolyG4(unsigned char * baseAddr)
{
	...
	if(iOffscreenDrawing)
	{
		...
		{
			InvalidateTextureAreaEx();   
            SOFT_CALL drawPoly4G(GETLE32(&gpuData[0]), GETLE32(&gpuData[2]), GETLE32(&gpuData[4]), GETLE32(&gpuData[6]));
		}     
	}
...
void primPolyFT3(unsigned char * baseAddr)
{
	...
	if(iOffscreenDrawing)
	{
		...
		if(bDrawOffscreen3())
		{
			InvalidateTextureAreaEx();   
			SetRenderColor(GETLE32(&gpuData[0]));
			SOFT_CALL drawPoly3FT(baseAddr);
		}
	}
...
uint32_t tp = GETLE32(&gpuData[4]) >> 16;
	...
	if(iOffscreenDrawing)
	{
		...
		if(bDrawOffscreen4())
		{
			InvalidateTextureAreaEx();   
			SetRenderColor(GETLE32(&gpuData[0]));
			SOFT_CALL drawPoly4FT(baseAddr);
		}
	}
...
{    
	...
	if(iOffscreenDrawing)
	{
		...
		{
			InvalidateTextureAreaEx();   
			SOFT_CALL drawPoly3GT(baseAddr);
		}
	}
...
{    
	...
	if(iOffscreenDrawing) 
	{
		...
		{
			InvalidateTextureAreaEx();   
            SOFT_CALL drawPoly3G(GETLE32(&gpuData[0]), GETLE32(&gpuData[2]), GETLE32(&gpuData[4]));
		}
	}
...
{ 
	...
	if(iOffscreenDrawing)
	{
		...
		{
			InvalidateTextureAreaEx();   
			SOFT_CALL drawPoly4GT(baseAddr);
		}     
	}
...
{    
	...
	if(iOffscreenDrawing)
	{
		...
		{
			InvalidateTextureAreaEx();   
			SOFT_CALL drawPoly3F(gpuData[0]);
		}
	}
    ...
    while (!(((GETLE32(&gpuData[i]) & 0xF000F000) == 0x50005000) && i >= 4)) {
		...
		if (bDraw && ((lx0 != lx1) || (ly0 != ly1)))
		{
			if(iOffscreenDrawing)
			{
				...
				{
					InvalidateTextureAreaEx();   
					SOFT_CALL drawPoly4G(gpuData[i-3],gpuData[i-1],gpuData[i-3],gpuData[i-1]);
				}
...
{    
	...
	if(iOffscreenDrawing)
	{
		...
		{
			InvalidateTextureAreaEx();   
			SOFT_CALL drawPoly4G(gpuData[0],gpuData[2],gpuData[0],gpuData[2]);
		}
	}
    ...
    while (!(((GETLE32(&gpuData[i]) & 0xF000F000) == 0x50005000) && i >= 3)) {
        ...
        if (!offsetline()) {
            if (iOffscreenDrawing) {
				...
				{
					InvalidateTextureAreaEx();   
					SOFT_CALL drawPoly4F(gpuData[0]);
				}
				lx0=cx0;lx1=cx1;ly0=cy0;ly1=cy1;
			}
